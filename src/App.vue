...
function parseSchema(txt: string): Record<string, string> {
  ...
  if (!obj || typeof obj !== "object")
    throw new Error("スキーマはオブジェクトで指定してください。");
...
function openApiToSchema(doc: any): Record<string, string> {
  ...
  if (schema.type === "array" && schema.items?.properties)
    return convertProps(schema.items.properties);
  if (schema.type === "object" && schema.properties)
    return convertProps(schema.properties);
...
function onOpenApiUpload(e: Event) {
  ...
  reader.onload = () => {
    try {
      ...
      const doc = /\.(ya?ml)$/i.test(file.name)
        ? yaml.load(text)
        : JSON.parse(text);
    ...
    <div class="grid md:grid-cols-2 gap-4">
      <div class="space-y-3">
        ...
        <div class="mt-4 space-y-2">
          ...
          <input
            type="file"
            @change="onOpenApiUpload"
            accept=".yaml,.yml,.json"
          />
          <p class="text-xs text-gray-500">
            最初の path/method/200 の schema
            を読み取り、上のスキーマ欄に変換（MVP）。
          </p>
        </div>
      ...
      <div class="space-y-3">
        <div>
          ...
          <pre
            class="mt-1 w-full border rounded p-3 bg-white overflow-auto text-sm"
        ...
        <div>
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <button
                class="px-3 py-1.5 rounded bg-slate-600 text-white text-sm"
                @click="copyMsw"
              >
                コピー
              </button>
              <button
                class="px-3 py-1.5 rounded bg-slate-800 text-white text-sm"
                @click="downloadMsw"
                :disabled="!result"
              >
                ダウンロード
